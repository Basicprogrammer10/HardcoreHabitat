    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher,
        ...
        dispatcher.register(CommandManager.literal("lives")
                .then(CommandManager.argument("player", word())
                        .suggests((c, b) -> suggestMatching(players(), b))
                        .executes(LivesCommand::run)));

        // Transfer life command
        dispatcher.register(CommandManager.literal("transfer-life")
                .then(CommandManager.argument("player", word())
                        .suggests((c, b) -> suggestMatching(Arrays.stream(players())
                                .filter(x -> !Objects.equals(x,
                                        Objects.requireNonNull(c.getSource()
                                                        .getPlayer())
                                                .getName()
                                                .getString())), b))
